...
name: prerequisites
description: >-
  Pre-flight for the suite repo — verifies every hard requirement (tools,
  MCPs, credentials) before any content flow runs. USE THIS as step 0 of
  /create-suite, when the user asks "check prerequisites" / "am I set up",
...
# prerequisites — suite repo pre-flight
...
`--launch` is passed to claude, so `--launch -p "make suite x"` starts a
...
- All ✅ → the checklist, then `READY — all suite-repo prerequisites
  satisfied.`
