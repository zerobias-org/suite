...
// Local enum for suite status validation
const VspStatusEnum = {
...
const suiteTypes: Record<string, number> = {};
...
function processPackageJson(packageFile: Record<string, any>, code: string, vendor: string): void {
  let check: any = packageFile.name !== undefined && packageFile.name !== null && packageFile.name === `@zerobias-org/suite-${vendor}-${code}`
    ? true : new Error('package.json missing name or not set to @zerobias-org/suite-<vendor>-<code>');
  ...
  if (packageFile.auditmation && typeof packageFile.auditmation === 'object') {
    ...
    check = auditmation['import-artifact'] !== undefined && auditmation['import-artifact'] !== null && auditmation['import-artifact'] === 'suite'
      ? true : new Error('package.json auditmation section missing import-artifact or not set to suite');
